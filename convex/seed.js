...
export const seedDatabase = mutation({
  ...
  handler: async (ctx) => {
    ...
    const settlements = await createSettlements(ctx, users, groups, oneOnOneExpenses, groupExpenses);
...
async function createSettlements(ctx, users, groups, oneOnOneExpenses, groupExpenses) {
  const now = Date.now();
  ...
  const cabExpense = oneOnOneExpenses.find((expense) => expense.description === "Cab ride to airport");

  // Find some group expenses to settle
  const hotelExpense = groupExpenses.find((expense) => expense.description === "Hotel reservation");

  const coffeeExpense = groupExpenses.find((expense) => expense.description === "Coffee and snacks");
