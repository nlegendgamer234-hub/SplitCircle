...
export function CreateGroupModal({ isOpen, onClose, onSuccess }) {
  ...
  const addMember = (user) => {
    ...
    if (!selectedMembers.some((m) => getUserId(m) === userId)) {
  ...
  const removeMember = (userId) => {
    setSelectedMembers(selectedMembers.filter((m) => getUserId(m) !== userId));
  };

  const onSubmit = async (data) => {
    try {
      const memberIds = selectedMembers.map((member) => getUserId(member));
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2 mb-2">
              ...
              {selectedMembers.map((member) => {
                ...
                return (
                  <Badge key={memberId} variant="secondary" className="px-3 py-1">
                    ...
                    <button type="button" onClick={() => removeMember(memberId)} className="ml-2 text-muted-foreground hover:text-foreground">
                      <X className="h-3 w-3" />
                    </button>
                  </Badge>
                );
              ...
              {/* FIXED: Removed nested <Button> and asChild prop */}
              <Popover open={commandOpen} onOpenChange={setCommandOpen}>
                <PopoverTrigger className={cn(buttonVariants({ variant: "outline", size: "sm" }), "h-8 gap-1 text-xs")}>
                    ...
                    <CommandList>
                      ...
                      <CommandGroup heading="Users">
                        {searchResults?.map((user) => {
                          const userId = getUserId(user);
                          return (
                            <CommandItem key={userId} value={`${user.name} ${user.email}`} onSelect={() => addMember(user)}>
            ...
            {selectedMembers.length === 0 && <p className="text-sm text-amber-600">Add at least one other person to the group</p>}
          </div>
