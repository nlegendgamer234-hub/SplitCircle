  ...
  return (
    <div className="container mx-auto py-6 max-w-lg">
      ...
      <div className="mb-6">
        ...
        <p className="text-muted-foreground mt-1">{type === "user" ? `Settling up with ${data?.counterpart?.name}` : `Settling up in ${data?.group?.name}`}</p>
      </div>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center gap-3">
            ...
            <CardTitle>{type === "user" ? data?.counterpart?.name : data?.group?.name}</CardTitle>
          </div>
        </CardHeader>
        <CardContent>
          <SettlementForm entityType={type} entityData={data} onSuccess={handleSuccess} />
        </CardContent>
      </Card>
    </div>
  );
